...
/*
 * The model name used for the member role (typically 'Role' or 'Group')
 */
...
/*
 * The model name used for the member (typically 'Member')
 */
Configure :: write('acl.aro.member.model', 'Member');

/*
 * The primary key of the member model
 *
 * (can be left empty if your primary key's name follows CakePHP conventions)('id')
 */
Configure :: write('acl.aro.member.primary_key', '');
 ...
 * (This may prevent a member from being rejected from the ACL plugin after a ACL permission update)
 */
Configure :: write('acl.role.access_plugin_role_ids', array());

/*
 * You can add here  id(s) that are always allowed to access the ACL plugin (by bypassing the ACL check)
 * (This may prevent a member from being rejected from the ACL plugin after a ACL permission update)
 */
Configure :: write('acl.role.access_plugin_member_ids', array(1));

/*
 * The  table field used as membername in the views
 * It may be a table field or a SQL expression such as "CONCAT(Member.lastname, ' ', Member.firstname)" for MySQL or "Member.lastname||' '||Member.firstname" for PostgreSQL
 */
Configure :: write('acl.member.display_name', "Member.username");

/*
 * Indicates whether the presence of the Acl behavior in the member and role models must be verified when the ACL plugin is accessed
 */
...
/*
 * Indicates whether the  permissions page must load through Ajax
 */
Configure :: write('acl.gui._permissions.ajax', true);
?>