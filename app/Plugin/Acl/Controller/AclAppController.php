...
class AclAppController extends AppController
{
	...
	private function _check_config()
	{
		...
		if(!empty($role_model_name))
		{
	    	...
	    	$this->set('member_model_name',    Configure :: read('acl.aro.member.model'));
	    	$this->set('role_pk_name',       $this->_get_role_primary_key_name());
	    	$this->set('member_pk_name',       $this->_get_member_primary_key_name());
...
//	    	}
	    	...
	    	if(Configure :: read('acl.check_act_as_requester'))
	    	{
	    		...
	    		if(!$this->AclManager->check_member_model_acts_as_acl_requester(Configure :: read('acl.aro.member.model')))
	    		{
	    			$this->set('model_is_not_requester', false);
	    			$is_requester = false;
	    		}
	    		
	    		if(!$this->AclManager->check_member_model_acts_as_acl_requester(Configure :: read('acl.aro.role.model')))
	    		{
	...
	private function _authorize_admins()
	{
		...
		$authorized_member_ids = Configure :: read('acl.role.access_plugin_member_ids');

		$model_role_fk = $this->_get_role_foreign_key_name();
		
	    if(in_array($this->Auth->member($model_role_fk), $authorized_role_ids)
	       || in_array($this->Auth->member($this->_get_member_primary_key_name()), $authorized_member_ids))
	    {
	...
	function _get_member_primary_key_name()
	{
	    $forced_pk_name = Configure :: read('acl.aro.member.primary_key');
	    if(!empty($forced_pk_name))
	    {
	        return $forced_pk_name;
	    }
