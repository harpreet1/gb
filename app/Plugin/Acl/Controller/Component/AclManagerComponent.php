...
class AclManagerComponent extends Component
{
	...
	public function check_member_model_acts_as_acl_requester($model_classname)
	{
...
//			 */
//			$member_model = ClassRegistry :: init($model_classname);
//		}
//		else
//		{
//			$member_model = $this->controller->{$model_classname};
//		}
		
	    $member_model = $this->get_model_instance($model_classname);
	    
		$behaviors = $member_model->actsAs;
    ...
    */
	...
	public function save_permission($aro_nodes, $aco_path, $permission_type)
	{
	    if(isset($aro_nodes[0]))
	    {
	        ...
	        elseif($aro_nodes[0]['Aro']['model'] == Configure :: read('acl.aro.member.model'))
	        {
	            $pk_name = $this->controller->_get_member_primary_key_name();
	        }
    	    ...
    	    if($specific_permission_needed)
    	    {
    	        if($permission_type == 'allow' || $permission_type == 'grant')
    	        {
        	        ...
        	        {
        	            trigger_error(__d('acl', 'An error occured while saving the specific permission'), E_MEMBER_NOTICE);
        	            return false;
        	        }
    	        }
    	        else
    	    ...
    	    elseif(isset($specific_permission_right))
    	    {
            	...
            	if(!empty($aco_node))
            	{
        	        ...
        	        if($specific_permission !== false)
        	        {
        	            ...
        	            {
        	                trigger_error(__d('acl', 'An error occured while deleting the specific permission'), E_MEMBER_NOTICE);
        	                return false;
        	            }
        	        }
        	        else
        	        {
        	            ...
        	            trigger_error(__d('acl', 'The specific permission id could not be retrieved'), E_MEMBER_NOTICE);
        	            return false;
        	        }
            	}
            	else
            	{
    	             ...
    	             * As $specific_permission_right has a value, we should never fall here, but who knows... ;-)
    	             */
            	    trigger_error(__d('acl', 'The child ACO id could not be retrieved'), E_MEMBER_NOTICE);
        	        return false;
            	}
    	    }
    	    else
	    ...
	    {
	        trigger_error(__d('acl', 'Invalid ARO'), E_MEMBER_NOTICE);
	        return false;
	    }
	}
	...
	private function get_specific_permission_right($aro_node, $aco_path)
	{
        ...
        elseif($aro_node['Aro']['model'] == Configure :: read('acl.aro.member.model'))
        {
            $pk_name = $this->controller->_get_member_primary_key_name();
        }
	...
	/**
	 * Set the permissions of the authenticated member in Session
    ...
    {
        if(!$this->Session->check('Acl.permissions'))
        {
            $actions = $this->AclReflector->get_all_actions();
            ...
            if(!empty($member))
            {
                $member = array(Configure :: read('acl.aro.member.model') => $member);
                $permissions = array();
                ...
                foreach($actions as $action)
                {
                    ...
                    $permissions[$aco_path] = $this->Acl->check($member, $aco_path);
                }
                
                $this->Session->write('Acl.permissions', $permissions);
            }
        }
    }
}