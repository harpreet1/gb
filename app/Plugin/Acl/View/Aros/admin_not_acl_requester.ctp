...
<div class="error">
	...
	if(isset($model_is_not_requester))
	{
		...
		echo sprintf(__d('acl', 'The <em>%s</em> model is not configured to act as an <em>ACL requester</em>'), Configure :: read('acl.aro.member.model')) . '</p>';
		echo '</p>';
	}
	...
	echo '<p>';
	echo sprintf(__d('acl', 'In a classical ACL configuration, the models that represent the  and the roles must act as <em>ACL requesters</em> (see %s).'), 
