...
$(function(){
	$('#cropbox').Jcrop({
		trueSize: [<?php echo $srcfilewidth ;?>, <?php echo $srcfileheight ;?>],
...
function showPreview(c){
	...
	if (parseInt(c.w) > 0){
		...
		$('#preview').css({
			width: Math.round(rx * <?php echo $srcfilewidth ;?>) + 'px',
			height: Math.round(ry * <?php echo $srcfileheight ;?>) + 'px',
...
<table>
	<tr>
		<td><?php echo $this->Html->image($sourcefileweb.'?'.date('now'), array('id' => 'cropbox', 'style' => 'float: left; margin-right: 10px;', 'width' => $width)); ?></td>
	</tr>
	<tr>
		<td>
			<br /><br />
	...
	<tr>
		<td><div style="width:<?php echo $width; ?>px;height:<?php echo $height; ?>px;overflow:hidden;"><?php echo $this->Html->image($sourcefileweb.'?'.date('now'), array('id' => 'preview')); ?></div></td>
	</tr>
</table>
...
<input type="hidden" name="data[Picture][destinationfile]" value="<?php echo $destinationfile; ?>" />
