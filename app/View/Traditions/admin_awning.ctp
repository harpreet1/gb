<?php
if(!empty($user['Tradition']['awning_hslv'])) {
	$hslv = explode(',', $user['Tradition']['awning_hslv']);
} else {
	$hslv = array(0,0,0,0);
}
?>
...
function refreshSwatch() {
	...
	var hue = $("#hue").slider("value"),
		...
		$('#TraditionAwningCss').html(embeddedCss);
		$('.ui-slider-handle').each(function(){
			...
			code = hslvcode;
			hslvcode =	code + v + ',';
			$('#TraditionAwningHslv').val(hslvcode);
		});
}
...
$(document).ready(function(){
	...
	$('#TraditionAwningCss').hide();
	$('#TraditionAwningHslv').hide();
...
<h2>International Traditions Awning Color Tool</h2>

<?php //debug($traditions) ?>
...
<div class="awning custom large" id="awning1"><img id="awning1" class="awning custom large" src="/img/awning/awning.png">
	...
	<!--<div id="div1">
		<div id="div2">
			<?php //echo $this->Html->image('regions/awnings/'. $tradition['Tradition']['awning_image']); ?>
		</div>​
	</div>
-->
</div>
...
<div style="margin-left:120px;">
	<?php echo $this->Html->image('regions/'. $tradition['Tradition']['image']); ?>
</div>
